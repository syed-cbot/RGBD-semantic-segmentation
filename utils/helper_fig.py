...
#######################################################
    ...
    pre_1 = np.array([i.argmax(axis=0) for i in pre_1])
    pre_1 = np.concatenate(pre_1)
    pre_1 = np.array([cmap[i.astype(np.uint8)+1] for i in pre_1])   

    pre_2 = np.array([i.argmax(axis=0) for i in pre_2])
    pre_2 = np.concatenate(pre_2)
    pre_2 = np.array([cmap[i.astype(np.uint8)+1] for i in pre_2]) 

    pre_3 = np.array([i.argmax(axis=0) for i in pre_3])
    pre_3 = np.concatenate(pre_3)
    pre_3 = np.array([cmap[i.astype(np.uint8)+1] for i in pre_3]) 

    pre_4 = np.array([i.argmax(axis=0) for i in pre_4])
    pre_4 = np.concatenate(pre_4)
    pre_4 = np.array([cmap[i.astype(np.uint8)+1] for i in pre_4]) 
############################################################
    img = img.transpose(1, 2, 0)
    pre_depth

    return np.concatenate([img, lab,pre_1,pre_2,pre_3,pre_4,pre], 1)


def make_validation_img(img_, lab, pre, pre_1):
    # img (image): (np.array) batchsize * 3 * H * W
    # lab (label): (np.array) batchsize * H * W
    # pre (predition): (np.array) batchsize * H * W
...
############################################################
    ...
    return np.concatenate([img, lab,pre_1,pre], 1)
