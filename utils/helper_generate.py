...
def make_validation_img(img_, lab, pre, depth, predict_depth):
    ...
    predict_depth*= 255
    predict_depth = np.concatenate(predict_depth, axis=1)
    ...
    predict_depth = predict_depth.transpose(1, 2, 0)
    #print (img.shape,lab.shape,pre.shape,depth.shape,predict_depth.shape)
    return np.concatenate([img,depth, lab, pre, predict_depth], 1)
