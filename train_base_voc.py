...
from dataloader.voc_generate import VOC
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', default='VOC', help='NYU or SUN or CITY or VOC', type=str)
    args = parser.parse_args()
    save_dir = './%s_base/'%args.dataset
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
        
    epochs = {'SUN':60,'NYU':300,'CITY':200,'VOC':200}[args.dataset]

    train_dataset = VOC(phase = 'train')
    train_loader = DataLoader(
            train_dataset,
            batch_size=batch_size,
            shuffle=True,
            num_workers=workers)
    val_dataset = VOC(phase = 'val')
    val_loader = DataLoader(
            val_dataset, batch_size=batch_size, shuffle=False, num_workers=workers)

    num_class = {'SUN':37,'NYU':40,'CITY':19,'VOC':21}[args.dataset]
    ignore_label = {'SUN':255,'NYU':255,'CITY':250,'VOC':255}[args.dataset]
    loss = nn.CrossEntropyLoss(ignore_index=ignore_label)
    patience = {'SUN':15,'NYU':60,'CITY':40,'VOC':40}[args.dataset]
