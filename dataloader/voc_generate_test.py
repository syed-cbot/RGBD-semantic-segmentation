...
class VOC(Dataset):
    def __init__(self, size = 480, phase='train'):
        root = '/home/gzx/RGBD/'
        ...
        def base_transform(t='img'):
            ...
            return {
                'train': transforms.Compose([
                    ...
                    transforms.RandomResizedCrop(self.size, scale=(0.8, 1.5), ratio=(1.2, 0.85), interpolation=interpolation),
                    #transforms.RandomCrop((480,640), padding=0),
                    transforms.RandomHorizontalFlip(),
                ]),
                'val': transforms.Compose([
                    transforms.Resize((int(0.625*self.size),self.size), interpolation=interpolation),
                ]),
    ...
    def __getitem__(self, idx):
        ...
        if self.phase =='test':
        	label = None
        	sample = {
                'image': image, 'depth': depth, 'shape': shape
            }
        else:
