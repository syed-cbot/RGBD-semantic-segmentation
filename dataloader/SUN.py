...
class SUNRGBDLoader(data.Dataset):
    ...
    def __init__(
        ...
        print (self.root + self.split + "_image/")
        for split in ["train", "test"]:
            file_list = sorted(
                recursive_glob(rootdir=self.root + self.split + "_image/", suffix="jpg")
            )
        ...
        for split in ["train", "test"]:
            file_list = sorted(
                recursive_glob(
                    rootdir=self.root + "sunrgbd-meta-data/labels/" + self.split + "/", suffix="png"
                )
            )
    ...
    def __getitem__(self, index):
        #print (self.files[self.split][index],self.anno_files[self.split][index])
    ...
    def transform(self, img, lbl):
        ...
        #assert np.array(lbl).max()<38
        #img = transforms.ToPILImage()(img.astype(np.uint8))
        #img = transforms.Resize((640, 640))(img)
        #lbl = transforms.ToPILImage()(lbl[:,:,np.newaxis])
        #lbl = transforms.Resize((640, 640))(lbl)
